...
def _pw(kp, i, conf):
    """MediaPipe world point [x,y,z,visibility], when reliable."""
    return np.asarray(kp[i][:3], float) if kp and kp[i][3] >= conf else None
...
def compute_reba_pose33(kp, confidence=0.35, world=None):
    """Compute REBA components, preferring relative 3D world landmarks."""
    p2 = lambda i: _p(kp, i, confidence)
    pw = lambda i: _pw(world, i, confidence)
    # Do not mix pixel-space 2D points with metre-scale 3D points in one angle.
    p = pw if world else p2
    ...
    dimensions = len(shoulders)
    up = np.array([0., -1., 0.]) if dimensions == 3 else np.array([0., -1.])
    down = -up
    ...
    required = {
        "neck_deg": neck, "trunk_deg": trunk, "upper_arm_deg": upper,
    ...
    return {
        ...
        "measurement_space": "mediapipe_world_3d" if world else "image_2d",
    }
